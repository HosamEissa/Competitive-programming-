...
{
    return isPointOnRay(a, b, c) || isPointOnRay(b, a, c)||isPointOnRay(a, c, b)||isPointOnRay(c, a, b)||isPointOnRay(b, c, a)||isPointOnRay(c, b, a);
}
...
int main()
{
    ...
    while(t--){
        ...
        if(x1>x2)swap(x1,x2);
        if(y1>y2)swap(y1,y2);
        vp.push_back(point(min(x1,x2),min(y1,y2)));
        vp.push_back(point(min(x1,x2),max(y1,y2)));
        vp.push_back(point(max(x1,x2),min(y1,y2)));
        vp.push_back(point(max(x1,x2),max(y1,y2)));
        vp.push_back(point(min(x1,x2),min(y1,y2)));
        bool ans=0;
        for(int i=0;i<4;i++){
            point p1=vp[i];
            point p2=vp[i+1];
            if(intersect(st,en,p1,p2))ans=1;

        }

            if(st.X>=x1&&st.X<=x2&&st.Y>=y1&&st.Y<=y2||
                en.X>=x1&&en.X<=x2&&en.Y>=y1&&en.Y<=y2)
                    ans=1;
        if(ans)cout<<"T"<<endl;
        else cout<<"F"<<endl;
    }
